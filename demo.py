...
def main():
    ...
    with tab1:
        ...
        # 사용 가이드
        with st.expander("📖 사용 가이드 — 처음이시라면 여기를 펼쳐보세요!", expanded=False):
            st.markdown("""
## 🍌 PaperBanana란?
논문의 방법론 텍스트와 그림 캡션을 입력하면, AI 에이전트들이 협력하여 학술지 수준의 다이어그램을 자동으로 생성해주는 도구입니다.
...
### 1단계 — 방법론 텍스트 입력
논문의 **Method / Methodology 섹션** 내용을 붙여넣으세요.
- 마크다운 형식 권장 (`##`, `###` 헤딩, 수식 `$$...$$` 포함 가능)
- 처음이라면 **"예시 불러오기"** 드롭다운에서 `PaperBanana 프레임워크`를 선택해 테스트해보세요

### 2단계 — 그림 캡션 입력
생성할 그림의 캡션을 입력하세요.
- 예: `Figure 1: Overview of our proposed framework.`
- 캡션이 구체적일수록 더 정확한 다이어그램이 생성됩니다

### 3단계 — 후보 생성 클릭
🚀 **후보 생성** 버튼을 누르면 병렬로 여러 후보를 동시에 생성합니다.
- 기본 10개 후보 생성 (무료 API 사용 시 **3~5개** 권장)
- 생성 시간: 후보 1개당 약 3~10분 (병렬 처리로 전체 10~30분)
...
| 설정 | 권장값 | 설명 |
|------|--------|------|
| **파이프라인 모드** | `demo_full` | 5개 에이전트 전체 사용 (최고 품질) |
| **후보 수** | `3~5개` | 무료 API 사용 시 과부하 방지 |
| **최대 평가 라운드** | `2~3` | 높을수록 품질↑ but 시간↑ |
| **종횡비** | `16:9` | 일반 논문 그림에 적합 |

### 파이프라인 모드 차이
- **demo_full**: Retriever → Planner → Stylist → Visualizer → Critic (더 미적, 단순화될 수 있음)
- **demo_planner_critic**: Planner → Visualizer → Critic (더 정확, 덜 스타일화)

---

## 🎨 결과 활용법

생성된 후보들 중 마음에 드는 것을 **⬇️ 다운로드** 하거나:
1. **이미지 개선 탭**으로 이동
2. 다운로드한 이미지 업로드
3. "색상을 더 학술적으로 바꿔줘" 등 수정 요청 입력
4. **2K/4K 고해상도**로 업스케일 가능

---

## 💡 팁
- 방법론 텍스트가 길고 구체적일수록 다이어그램 품질이 올라갑니다
- 같은 입력으로 여러 번 실행하면 매번 다른 스타일의 후보가 생성됩니다
- **"진화 타임라인 보기"** 를 펼치면 Planner → Stylist → Critic 각 단계의 변화를 확인할 수 있습니다
""")
...
The framework extends to statistical plots by adjusting the Visualizer and Critic agents. For numerical precision, the Visualizer converts the description $P_t$ into executable Python Matplotlib code: $I_t = \text{VLM}_{\text{code}}(P_t)$. The Critic evaluates the rendered plot and generates a refined description $P_{t+1}$ addressing inaccuracies or imperfections: $P_{t+1} = \text{VLM}_{\text{critic}}(I_t, S, C, P_t)$. The same $T=3$ round iterative refinement process applies. While we prioritize this code-based approach for accuracy, we also explore direct image generation in Section \ref{sec:discussion}. See Appendix \ref{app_sec:plot_agent_prompt} for adjusted prompts."""
    ...
    with tab2:
        ...
        with st.expander("📖 이미지 개선 사용법", expanded=False):
            st.markdown("""
### 사용 방법
1. **이미지 업로드** — 후보 생성 탭에서 다운로드한 PNG 또는 외부 다이어그램 이미지를 업로드합니다
2. **수정 지시사항 입력** — 원하는 변경 사항을 자연어로 설명합니다
3. **이미지 개선 클릭** — 2K 또는 4K 고해상도로 개선된 이미지를 생성합니다

### 수정 지시사항 예시
| 목적 | 입력 예시 |
|------|-----------|
| 단순 업스케일 | `모든 것을 그대로 유지하되 고해상도로 출력해줘` |
| 색상 변경 | `배경을 흰색으로 바꾸고 전체 색상을 학술지 스타일로 변경해줘` |
| 텍스트 수정 | `텍스트를 더 크고 굵게 만들어줘` |
| 스타일 변경 | `NeurIPS 논문 스타일의 깔끔한 다이어그램으로 바꿔줘` |
| 요소 추가 | `화살표를 더 명확하게 하고 각 단계에 번호를 추가해줘` |

### 해상도 선택
- **2K** (1440p): 대부분의 논문 제출에 적합, 생성 속도 빠름
- **4K** (2160p): 포스터·대형 출력용, 생성 시간 더 소요
""")
